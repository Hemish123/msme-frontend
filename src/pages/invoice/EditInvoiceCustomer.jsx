...
import { getInvoiceCustomer, updateInvoiceCustomer } from '../../api/invoiceCustomerAPI';
...
export default function EditInvoiceCustomer() {
  const { id } = useParams();
  ...
  useEffect(() => {
    (async () => {
      const { data, error } = await getInvoiceCustomer(id);
      if (data) {
        ...
        setForm({
          ...
          contact_person_1: payload.contact_person_1 || '',
          contact_person_2: payload.contact_person_2 || '',
          gst_number: payload.gst_number || '',
        });
      } else {
        toast.error(typeof error === 'string' ? error : 'Failed to load customer');
      }
      setFetching(false);
    })();
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const { data, error } = await updateInvoiceCustomer(id, form);
    setLoading(false);
    if (error) {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 lg:p-8">
      ...
      <h2 className="text-2xl font-bold text-slate-800 mb-6">Edit Customer</h2>

      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 lg:p-8 max-w-5xl">
        {fetching ? (
          <div className="h-64 flex items-center justify-center text-slate-500 animate-pulse">Loading customer details...</div>
        ) : (
